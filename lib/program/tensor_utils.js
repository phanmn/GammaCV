...
export function tensorInvert(
  ...
  const invert = new Function('coords', 'tmpArr', `${invertShape.map((a, key) => a ? `tmpArr[${key}] = shape[${key}] - 1 - coords[${key}]` : `tmpArr[${key}] = coords[${key}]`).join(';')}; return tmpArr;`); // eslint-disable-line
